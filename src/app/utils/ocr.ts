...
export async function runOCR(file: File): Promise<ParsedScore | null> {
  ...
  const parArray = parLine
    .split(/\s+/)
    .map((n) => parseInt(n, 10))
    .filter((n) => !isNaN(n));
  const imageSnippets: string[] = [];
  const scoreArray = await extractScoreNumbersFromCanvas(
    canvas,
    parArray.length,
    imageSnippets,
  );
  ...
  if (scoreArray.length !== parArray.length) {
    console.warn(
      `⚠️ Antall scorer (${scoreArray.length}) != antall hull (${parArray.length})`,
    );
  }
  ...
  const totalImageData = ctx.getImageData(
    totalCropX,
    totalCropY,
    totalCropWidth,
    totalCropHeight,
